      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-4">
          {vitals.length === 0 ? (
            ...
            vitals.map((record) => (
              <div key={record.id} className="bg-surface p-6 rounded-xl shadow-sm border border-border">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <span className="text-sm text-muted">{new Date(record.recordedAt).toLocaleString()}</span>
                    <p className="text-sm font-medium text-muted">Recorded by: {record.nurse.user.name}</p>
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                  <div className="p-3 bg-surface rounded-lg">
                    <span className="text-xs text-muted block">BP</span>
                    <span className="font-bold">{record.bloodPressure}</span>
                  </div>
                  <div className="p-3 bg-surface rounded-lg">
                    <span className="text-xs text-muted block">Heart Rate</span>
                    <span className="font-bold">{record.heartRate || '--'} bpm</span>
                  </div>
                  <div className="p-3 bg-surface rounded-lg">
                    <span className="text-xs text-muted block">O2 Sat</span>
                    <span className="font-bold">{record.oxygenSat || '--'}%</span>
                  </div>
                  <div className="p-3 bg-surface rounded-lg">
                    <span className="text-xs text-muted block">Temp</span>
                    <span className="font-bold">{record.temp || '--'} °C</span>
                  </div>
                  <div className="p-3 bg-surface rounded-lg">
                    <span className="text-xs text-muted block">Glucose</span>
                ...
                {record.notes && (
                  <div className="mt-4 p-3 bg-brand-primary/5 rounded-lg border border-brand-primary/20">
                    <span className="text-xs text-brand-primary font-bold block mb-1">Nurse Notes</span>
